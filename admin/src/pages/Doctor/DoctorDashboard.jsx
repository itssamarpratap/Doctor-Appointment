  ...
  return dashData && (
    <div className='m-5'>
      <div className='flex flex-wrap gap-3'>
        ...
        {/* Appointments Count (Excludes Cancelled) */}
        <div className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
        ...
        <div className='pt-4 border border-t-0'>
          {dashData.latestAppointments.slice(0, 10).map((item, index) => (
            <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
              {/* Numbering */}
              <p className='text-gray-600 font-medium w-6'>{index + 1}.</p>
              ...
              <div className='flex-1 text-sm'>
                ...
                <p className='text-gray-600'>
                  Booking on {slotDateFormat(item.slotDate)} at {item.slotTime} {/* Slot Time */}
                </p>
              </div>

              {/* Status Handling */}
              {item.cancelled ? (
                <p className='text-red-400 text-xs font-medium'>Cancelled Slot</p> // Showing slot cancellation
              ) : item.isCompleted ? (
